...
def load_agnes_key() -> str:
    """Load AGNES_API_KEY from env or .env.local."""
...
Read the ENTIRE segment and do two things:
...
- expected_behavior: what the PABERIN AI ASSISTANT should do when a customer sends this inquiry: "quote" (enough detail to quote now), "clarify" (missing info — ask questions first), "redirect" (off-topic/not a service), "confirm" (confirm/close an order or booking), "other".
...
def load_live_system_prompt() -> str:
    """Extract PABERIN_SYSTEM_PROMPT from src/lib/chat.ts so ideal responses
    are generated under the SAME instructions the deployed assistant uses."""
    path = REPO_ROOT / "src" / "lib" / "chat.ts"
    text = path.read_text(encoding="utf-8")
    m = re.search(r"export const PABERIN_SYSTEM_PROMPT = `([\s\S]*?)`;", text)
    if not m:
        log("WARNING: could not extract PABERIN_SYSTEM_PROMPT from src/lib/chat.ts")
        return ""
    return m.group(1)
...
def pass2_ideal_response(inquiry: dict, system_prompt: str) -> Optional[str]:
    ...
    user_content = (
        ...
        f"What Paberin actually quoted in the chat: {json.dumps(inquiry.get('what_paberin_quoted', {}))}\n\n"
...
def finalize() -> None:
    ...
    pass1 = load_ckpt(CKPT_DIR / "pass1.jsonl")
    pass2 = load_ckpt(CKPT_DIR / "pass2.jsonl")
    ...
    out = OUTPUT_DIR / "paberin_chat_eval_dataset.json"
    out.write_text(json.dumps(dataset, ensure_ascii=False, indent=2), encoding="utf-8")

    analyses_out = OUTPUT_DIR / "paberin_chat_analyses.json"
...
def main() -> None:
    log("=" * 72)
    log("PABERIN CHAT DATASET GENERATOR v3.0 — Agnes-powered pipeline")
    log("=" * 72)
