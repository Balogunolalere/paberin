...
/** Minimal inline icons (this project has no lucide-react dependency). */
  ...
  useEffect(() => {
    ...
    loadMapboxGL()
      .then((mapboxgl) => {
        ...
        const marker = new mapboxgl.Marker({ color: '#FF5C00', draggable: true })
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        />
        {searching && <LoaderIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#FF5C00] animate-spin" />}
        {showSug && suggestions.length > 0 && (
          <ul className="absolute z-20 mt-1 w-full bg-white border border-[#EAEAEA] shadow-lg max-h-56 overflow-y-auto">
            {suggestions.map((s, i) => (
              <li key={i}>
                <button
                  type="button"
                  onMouseDown={() => pick(s.place, s.center)}
                  className="w-full text-left px-4 py-2.5 text-sm text-black hover:bg-[#F7F7F7] flex items-start gap-2"
                >
                  <PinIcon className="w-3.5 h-3.5 mt-0.5 shrink-0 text-[#888888]" />
      ...
      {mapState === 'error' ? (
        <p className="text-[11px] text-[#888888] border border-[#EAEAEA] p-3">
          The map could not be loaded — you can still pick an address from the search results above.
        </p>
      ) : (
        <div className="relative">
          <div ref={containerRef} className="h-44 w-full rounded-lg border border-[#EAEAEA]" data-testid="address-picker-map" />
          {mapState === 'loading' && (
            <div className="absolute inset-0 flex items-center justify-center text-xs text-[#888888] bg-white/60">
              Loading map…
            </div>
          )}
        </div>
      ...
      {/* Chosen address */}
      {value && (
        <p className="text-[11px] text-[#666666] flex items-start gap-1.5">
          <PinIcon className="w-3.5 h-3.5 mt-0.5 shrink-0 text-[#FF5C00]" />
