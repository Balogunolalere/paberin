 ...
 *
 * All data flows through the shared `api` client in `src/lib/api.ts`:
 *   - api.getEscalationThread(ticketId, phone)  → { ticketId, orderNumber, status, messages[] }
 *   - api.replyEscalation(ticketId, { phone, message, customerName }) → { message }
 */
...
import {
  ...
  type EscalationThread as EscalationThreadData,
} from '@/lib/api';
...
interface EscalationThreadProps {
  ...
  phone: string;
  customerName?: string;
  /** Called when the thread transitions to RESOLVED (best-effort). */
  onResolved?: () => void;
}
...
function fullName(m: EscalationMessage): string {
  ...
  if (role === 'admin') return 'Paberin team';
...
export function EscalationThread({ ticketId, phone, customerName, onResolved }: EscalationThreadProps) {
  ...
  const loadThread = useCallback(async () => {
    ...
    try {
      const data = await api.getEscalationThread(ticketId, phone);
      setThread(data);
      // Fire onResolved once when the ticket transitions to RESOLVED.
      if (data?.status?.toUpperCase() === 'RESOLVED' && !resolvedFiredRef.current) {
  ...
  const sendReply = async () => {
    ...
    // Optimistic append — build a temporary CUSTOMER message and prepend it
    ...
    try {
      const res = await api.replyEscalation(ticketId, { phone, message, customerName });
      // Replace the optimistic bubble with the canonical one returned by the API.
      const canonical = (res as any)?.message ?? null;
      setThread((prev) => {
        if (!prev) return prev;
  ...
  return (
    <div className="flex flex-col h-[60vh] min-h-[360px]">
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 bg-[#FAFAFA]"
      >
        {loading ? (
          <div className="flex items-center gap-3 justify-center py-10">
            ...
            <span className="text-sm text-[#666666]">Loading conversation…</span>
          </div>
        ) : error ? (
          ...
          messages.map((m) => {
            ...
            if (role === 'SYSTEM') {
              return (
                <div key={m.id} className="flex justify-center my-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-[#888888] italic">
                    {m.body}
                  </span>
                </div>
              );
            ...
            return (
              ...
              >
                <div
                  className={`max-w-[80%] px-3.5 py-2.5 rounded-lg ${
                    isCustomer
                      ? 'bg-[#F2F2F2] text-black rounded-bl-sm'
                      : 'bg-[#FF5C00] text-white rounded-br-sm'
                  }`}
                >
                  <p className="text-[13px] leading-relaxed whitespace-pre-wrap break-words">{m.body}</p>
                </div>
                <p
                  className={`font-mono text-[10px] uppercase tracking-[0.1em] text-[#888888] mt-1 ${
      ...
      <div className="border-t border-[#EAEAEA] p-3 sm:p-4 bg-white">
        {sendError && (
          <p className="text-xs text-[#E05200] mb-2">{sendError}</p>
        )}
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="form-input resize-none flex-1 disabled:opacity-60 disabled:cursor-not-allowed"
          />
          <button
            onClick={sendReply}
