...
export function parseSpecsBlock(text: string): ChatSpecs | undefined {
  ...
  // are owner-authored so they are not uniformly cased — the live catalog
  // contains `paberin_plain_cardboard_cake_Topper`. Lowercasing it here made
  // that service unpriceable; the route resolves the canonical key instead.
  const serviceType =
