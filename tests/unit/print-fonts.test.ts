...
describe('the order form renders a font as its own input', () => {
  it('maps a font field to the font picker, not a plain dropdown', () => {
    const field: OptionField = { key: 'fonts', label: 'Font', type: 'font', choices: ['Clarendon'] };
    const model = optionInputModel(field);
    expect(model.kind).toBe('font');
    expect(model.choices.map((c) => c.value)).toEqual(['Clarendon']);
  });

  it('still maps a dropdown to a select', () => {
    expect(optionInputModel({ key: 'c', label: 'Colour', type: 'dropdown', choices: ['Gold'] }).kind).toBe('select');
  });

  it('carries required through, so the picker can mark it', () => {
    expect(optionInputModel({ key: 'f', label: 'Font', type: 'font', required: true }).required).toBe(true);
  });
});
...
describe('the free stand-in chosen for each font', () => {
  ...
  it('lists exactly those families for the stylesheet, with no orphans', () => {
    ...
    expect(Array.from(PREVIEW_FALLBACK_FAMILIES).sort()).toEqual(Array.from(new Set(matches)).sort());
  });
